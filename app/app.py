...
def parse_invoice_data(text, pdf_path=None):
    ...
    if receiver_gst_match:
        ...
        if all_gsts:
             if len(all_gsts) > 1:
                 ...
                 # Only one GSTIN found - it's likely the sender's, so mark receiver as 'not found'
                 common_data['receiver_gst'] = 'not found'
        else:
             # No GSTIN found in Bill To section - mark as 'not found' for unregistered customers
             common_data['receiver_gst'] = 'not found'
                ...
                for page in pdf.pages:
                    ...
                    for table in tables:
                        ...
                        for row_idx, row in enumerate(table):
                            ...
                            # Valid item row: Accept items even without HSN codes
                            # Items without HSN will be marked as 'not found' and captured in Excel
                            # Check: has SNO or any meaningful data in the row (qty, rate, amount)
                            has_meaningful_data = (qty_val and str(qty_val).strip()) or (rate_val and str(rate_val).strip()) or (invoice_val and str(invoice_val).strip())
                            if has_meaningful_data:
                                item_data = {
                                    'sno': str(sno_val).strip() if sno_val else str(len(all_items) + 1),
                                    'hsn': str(hsn_val).strip() if hsn_val and str(hsn_val).strip() else 'not found',
                                    'quantity': '',
                                    'taxable_amount': '',
                                    'discount': '',
                                    'taxrate': common_data.get('taxrate', ''),  # Use actual GST rate from PDF, empty if not found
                                    'cgst': '',
                                    'sgst': '',
                                    'invoice_value': ''
                                }
    ...
    if all_items:
        ...
        for item in all_items:
            hsn = item['hsn']
            # Process all items including those with 'not found' HSN
            # Items without HSN will be grouped under 'not found' key
